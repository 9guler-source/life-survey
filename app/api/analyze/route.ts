...
import { questionsEN, questionsJA, LangCode } from "@/app/data/i18n";
...
function buildPrompt(
  ...
  const genderLabel = getGenderLabel(gender, lang);
...
[執筆指針]
1. 回答者に直接語りかけるように2人称（「あなた」）で書いてください。
2. 過去（歩んできた道）・現在（今この瞬間）・未来（これからの旅）の三つの流れで構成してください。
3. 具体的な回答内容を織り込んで個人化された解説を書いてください。
4. 回答者の年齢と文化的背景を自然に反映してください。
5. 感動的で温かく、しかし陳腐にならないように書いてください。
6. 最後に今日すぐ実践できる小さな行動を一つ提案してください。
7. 600〜900文字で書いてください。
8. タイトルなしで本文から始め、絵文字や特殊記号を使わず純粋な文章で書いてください。`,

    "zh-hans": `您是一位温暖而深刻的人生顾问和作家。
以下是一位来自${nationality}、${age}岁${genderLabel}完成"我的人生罗盘"问卷的40个回答。
请充分考虑此人的年龄、性别、国籍和文化背景，用中文（简体）为其撰写专属的个性化人生解读。
...
[寫作指南]
1. 以第二人稱（「您」）直接向受訪者說話。
2. 圍繞過去（走過的路）、現在（此刻）、未來（前方的旅程）三個流程自然展開。
3. 融入具體的回答內容，創作個性化的解讀。
4. 自然地反映受訪者的年齡和文化背景。
5. 感人而溫暖，但不要流於俗套。
6. 最後提出一個今天就能付諸實踐的小行動。
7. 總字數在400至600字之間。
8. 直接從正文開始，不需要標題，不使用表情符號或特殊符號。`,

    es: `Eres un asesor de vida y escritor cálido y perspicaz.
A continuación se presentan 40 respuestas de una persona de ${nationality}, de ${age} años, género ${genderLabel}, que completó la encuesta 'Mi Brújula de Vida'.
Por favor, escribe en español una narrativa de vida profundamente personalizada, teniendo en cuenta su edad, género, nacionalidad y contexto cultural.
...
[Directrices]
1. Dirígete al encuestado directamente en segunda persona ('tú' o 'usted').
2. Organiza en torno a: pasado (el camino recorrido), presente (este momento), futuro (el viaje por delante).
3. Incorpora las respuestas específicas para crear una narrativa personalizada.
4. Refleja naturalmente la edad y el contexto cultural del encuestado.
5. Sé conmovedor y cálido, sin caer en clichés.
6. Termina con una pequeña acción concreta que puedan emprender hoy.
7. Escribe entre 400 y 600 palabras.
8. Comienza directamente con el cuerpo del texto, sin título ni emojis.`,

    fr: `Vous êtes un conseiller de vie chaleureux et perspicace, ainsi qu'un écrivain.
Voici 40 réponses d'une personne de nationalité ${nationality}, âgée de ${age} ans, de genre ${genderLabel}, ayant complété le questionnaire 'Ma Boussole de Vie'.
Veuillez rédiger en français une narration de vie profondément personnalisée, en tenant compte de son âge, genre, nationalité et contexte culturel.
...
[Directives]
1. Adressez-vous directement au répondant à la deuxième personne ('vous').
2. Organisez autour de : passé (le chemin parcouru), présent (ce moment), futur (le voyage à venir).
3. Intégrez les réponses spécifiques pour créer un récit personnalisé.
4. Reflétez naturellement l'âge et le contexte culturel du répondant.
5. Soyez émouvant et chaleureux, sans tomber dans les clichés.
6. Terminez par une petite action concrète qu'ils peuvent entreprendre aujourd'hui.
7. Rédigez entre 400 et 600 mots.
8. Commencez directement par le corps du texte, sans titre ni emoji.`,

    de: `Du bist ein einfühlsamer Lebensberater und Autor.
Im Folgenden findest du 40 Antworten einer ${age}-jährigen Person aus ${nationality}, Geschlecht ${genderLabel}, die den Fragebogen 'Mein Lebenskompass' ausgefüllt hat.
Bitte verfasse auf Deutsch eine tiefgründig personalisierte Lebenserzählung, unter Berücksichtigung von Alter, Geschlecht, Nationalität und kulturellem Hintergrund.
...
[Richtlinien]
1. Sprich die befragte Person direkt in der zweiten Person ('du' oder 'Sie') an.
2. Gliedere die Erzählung in: Vergangenheit, Gegenwart und Zukunft.
3. Webe spezifische Antworten ein, um eine persönliche Erzählung zu schaffen.
4. Reflektiere natürlich das Alter und den kulturellen Hintergrund.
5. Sei bewegend und warm, ohne klischeehaft zu sein.
6. Schließe mit einer kleinen konkreten Handlung, die die Person heute umsetzen kann.
7. Schreibe 400–600 Wörter.
8. Beginne direkt mit dem Fließtext, ohne Titel oder Emojis.`,

    ar: `أنت مستشار حياة ودود وبصير، وكاتب متمرس.
فيما يلي 40 إجابة من شخص يحمل جنسية ${nationality}، عمره ${age} عامًا، جنسه ${genderLabel}، أتمّ استبيان "بوصلة حياتي".
يُرجى كتابة سرد حياة شخصي عميق باللغة العربية، مع مراعاة العمر والجنس والجنسية والخلفية الثقافية للشخص.

[معلومات المشارك]
الجنسية: ${nationality} | العمر: ${age} عامًا | الجنس: ${genderLabel}

[الإجابات]
${summary}

[إرشادات الكتابة]
1. خاطب المشارك مباشرةً بضمير المخاطب ("أنت").
2. نظّم السرد حول: الماضي (الطريق الذي سلكته)، والحاضر (هذه اللحظة)، والمستقبل (الرحلة القادمة).
3. ضمّن الإجابات المحددة لتخصيص السرد.
4. عكس بشكل طبيعي عمر المشارك وخلفيته الثقافية.
5. اكتب بأسلوب مؤثر ودافئ دون السقوط في الابتذال.
6. اختم باقتراح إجراء صغير وملموس يمكن للشخص اتخاذه اليوم.
7. اكتب ما بين 400 و600 كلمة.
8. ابدأ مباشرةً بالنص دون عنوان أو رموز تعبيرية.`,
  };
...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = buildPrompt(lang as LangCode, nationality, birthYear, gender, summary);
